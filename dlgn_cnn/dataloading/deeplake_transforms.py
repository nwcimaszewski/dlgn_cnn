...
class SubsequenceByOpto(MovieTransform):
    ...
    def __call__(self, x):
        ...
        opto_signal = x[self.opto_key] # (t,), scalar sequence, as this transform is applied to individual items in a batch
        ...
        i = np.random.choice(viable_starts) # choose random start, such that optogenetic signal will be constant for entire 60 frames
        return x.__class__(
            **{
                k: getattr(x, k)[:, i : i + self.frames, ...] # (b,c,t,...)
